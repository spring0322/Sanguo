...
#pragma warning restore CS0414 // The field 'MainGame.IsFullScreen' is assigned but its value is never used
        ...
        protected override void LoadContent()
        {
            ...
            Platform.GraphicsDevice.DeviceReset += (sender, e) =>
            {
                ...
                // 注意：这里可能需要重建SpriteBatch，但要注意线程安全和资源释放
                // 暂时只记录日志，以便确认崩溃是否与此相关
                try 
                {
                   if (SpriteBatch != null && !SpriteBatch.IsDisposed)
                   {
                        System.Diagnostics.Debug.WriteLine($"[MainGame] Old SpriteBatch Device ID: {SpriteBatch.GraphicsDevice.GetHashCode()}"); 
                   }
                }
                catch (Exception ex)
                {
                    System.Diagnostics.Debug.WriteLine($"[MainGame] Error inspecting SpriteBatch during reset: {ex.Message}");
                }
            };
