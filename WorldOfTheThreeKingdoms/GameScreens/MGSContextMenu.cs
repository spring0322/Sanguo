    ...
    partial class MainGameScreen : Screen
    {
        ...
        private void HandleContextMenuResult(ContextMenuResult result)
        {
            ...
            switch (result)
            {
                ...
                case ContextMenuResult.Person_Appointment_ToggleRecommendation: //举荐开关
                    this.ToggleAdvisorRecommendation();
                    break;
                ...
                case ContextMenuResult.Person_Appointment_AppointStrategist: //任命军师 (从建筑菜单)
                    // 直接调用已有的军师任命逻辑
                    goto case ContextMenuResult.Person_Appointment_AppointAdvisor;

                case ContextMenuResult.Monarch_AdvisorManagement_AppointStrategist: //君主菜单-任命军师
                    // 直接调用已有的军师任命逻辑
                    goto case ContextMenuResult.Person_Appointment_AppointAdvisor;

                case ContextMenuResult.Monarch_AdvisorManagement_RecallStrategist: //君主菜单-罢免军师
                    // 直接调用已有的军师罢免逻辑
                    goto case ContextMenuResult.Person_Appointment_RecallAdvisor;
                ...
                case ContextMenuResult.Tactics_Destroy:
                    // 使用新的智能破坏系统
                    Session.MainGame.mainGameScreen.TriggerIntelligentDestroy();
                    break;

                case ContextMenuResult.Tactics_Instigate:
                    // 使用新的智能煽动系统
                    Session.MainGame.mainGameScreen.TriggerIntelligentInstigate();
                    break;

                case ContextMenuResult.Tactics_Gossip:
                    // 使用新的智能流言系统
                    Session.MainGame.mainGameScreen.TriggerIntelligentGossip();
                    break;

                case ContextMenuResult.Tactics_Assassinate:
                    // 使用新的智能暗杀系统
                    Session.MainGame.mainGameScreen.TriggerIntelligentAssassinate();
                    break;
                ...
                case ContextMenuResult.Tactics_JailBreak:
                    // 触发军师建议，后续处理自动化
                    Session.MainGame.mainGameScreen.TriggerAdvisorAdvice(GameManager.StrategyKind.JailBreak);
                    break;
