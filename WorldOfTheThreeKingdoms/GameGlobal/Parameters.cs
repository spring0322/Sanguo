...
namespace GameGlobal
{
    ...
    public class Parameters
    {
        ...
        [DataMember]
        public AIDifficulty AIDifficulty = AIDifficulty.Normal; // Default to Normal
        [DataMember]
        ...
        [DataMember]
        public float AIWeightUnificationAggression = 1.5f;
        [DataMember]
        public float AIWeightSelfPreservationDefense = 1.8f;
        [DataMember]
        public int AIWeightEliteTroopGradeThreshold = 3;
        [DataMember]
        public float AIWeightExpToPowerRatio = 0.001f;
        [DataMember]
        public int AIWeightHanLoyaltyAttackThreshold = 60;

        [DataMember]
        public bool AIUseNewFormationLogic = true;
        ...
        public void InitializeGameParameters(string str = "")
        {
            ...
            if (str == string.Empty)
            {
                ...
                if (nextSibling.Attributes.GetNamedItem("AIWeightUnificationAggression") != null) AIWeightUnificationAggression = float.Parse(nextSibling.Attributes.GetNamedItem("AIWeightUnificationAggression").Value);
                if (nextSibling.Attributes.GetNamedItem("AIWeightSelfPreservationDefense") != null) AIWeightSelfPreservationDefense = float.Parse(nextSibling.Attributes.GetNamedItem("AIWeightSelfPreservationDefense").Value);
                if (nextSibling.Attributes.GetNamedItem("AIWeightEliteTroopGradeThreshold") != null) AIWeightEliteTroopGradeThreshold = int.Parse(nextSibling.Attributes.GetNamedItem("AIWeightEliteTroopGradeThreshold").Value);
                if (nextSibling.Attributes.GetNamedItem("AIWeightExpToPowerRatio") != null) AIWeightExpToPowerRatio = float.Parse(nextSibling.Attributes.GetNamedItem("AIWeightExpToPowerRatio").Value);
                if (nextSibling.Attributes.GetNamedItem("AIWeightHanLoyaltyAttackThreshold") != null) AIWeightHanLoyaltyAttackThreshold = int.Parse(nextSibling.Attributes.GetNamedItem("AIWeightHanLoyaltyAttackThreshold").Value);
                if (nextSibling.Attributes.GetNamedItem("AIUseNewFormationLogic") != null) AIUseNewFormationLogic = bool.Parse(nextSibling.Attributes.GetNamedItem("AIUseNewFormationLogic").Value);
            }
                ...
                for (int i = 0; i < nextSibling.Attributes.Count; i++)
                {
                    ...
                    foreach (var v in 非getset字段表)
                    {
                        if (v.Name == nextSibling.Attributes[i].Name)
                        {
                            ...
                            {
                                 try {
                                     object enumVal = Enum.Parse(v.FieldType, nextSibling.Attributes[i].Value, true);
                                     v.SetValue(Session.Current.Scenario.Parameters, enumVal);
                                 } catch {}
                            }
        ...
        public void SaveToXml()
        {
            ...
            element.SetAttribute("AIDifficulty", AIDifficulty.ToString());
    ...
    element.SetAttribute("AIUseNewFormationLogic", AIUseNewFormationLogic.ToString());
            ...
            element.SetAttribute("AIWeightUnificationAggression", AIWeightUnificationAggression.ToString());
            element.SetAttribute("AIWeightSelfPreservationDefense", AIWeightSelfPreservationDefense.ToString());
            element.SetAttribute("AIWeightEliteTroopGradeThreshold", AIWeightEliteTroopGradeThreshold.ToString());
            element.SetAttribute("AIWeightExpToPowerRatio", AIWeightExpToPowerRatio.ToString());
            element.SetAttribute("AIWeightHanLoyaltyAttackThreshold", AIWeightHanLoyaltyAttackThreshold.ToString());
            element.SetAttribute("AIUseNewFormationLogic", AIUseNewFormationLogic.ToString());
