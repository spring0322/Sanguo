    ...
    public class MenuItem
    {
            ...
            if (this.Visible)
            {
                ...
                if (this.selected || this.open)
                {
                    ...
                    else if (this.HasChild)
                    {
                        ...
                        {
                            // 修复：使用绝对值确保尺寸为正数
                            int textureWidth = Math.Abs(this.contextMenu.HasChildTexture.Width);
                            int textureHeight = Math.Abs(this.contextMenu.HasChildTexture.Height);
                            
                            CacheManager.Draw(this.contextMenu.HasChildTexture, StaticMethods.CenterRectangle(new Rectangle(this.Position.Left - textureWidth, this.Position.Top, textureWidth, this.Position.Height), new Rectangle(0, 0, textureWidth, textureHeight)), null, this.Enabled ? Color.White : this.contextMenu.RightDisabledTextColor, 0f, Vector2.Zero, SpriteEffects.None,0.04f);
                        }
                    }
                }
        ...
        public void LoadFromXmlNode(XmlNode rootNode)
        {
            ...
            foreach (XmlNode node in rootNode)
            {
                ...
                if (node.NodeType == System.Xml.XmlNodeType.Element && node.Name == "MenuItem")
                {
