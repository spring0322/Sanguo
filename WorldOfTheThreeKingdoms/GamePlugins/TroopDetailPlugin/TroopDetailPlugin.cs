...
namespace TroopDetailPlugin
{
	public class TroopDetailPlugin : GameObject, ITroopDetail, IBasePlugin, IPluginXML, IPluginGraphics
	{
		...
		public void LoadDataFromXMLDocument(string filename)
		{
            ...
            // Manually add Status label
            if (this.troopDetail.LabelTexts.Count > 0)
            {
                LabelText last = this.troopDetail.LabelTexts[this.troopDetail.LabelTexts.Count - 1];
                int dy = 25; // Default spacing
                if (this.troopDetail.LabelTexts.Count >= 2)
                {
                    LabelText prev = this.troopDetail.LabelTexts[this.troopDetail.LabelTexts.Count - 2];
                    dy = last.Label.Position.Y - prev.Label.Position.Y;
                }
                
                LabelText statusLabel = new LabelText();
                statusLabel.Label = new FreeText(font, color);
                statusLabel.Label.Position = new Rectangle(last.Label.Position.X, last.Label.Position.Y + dy, last.Label.Position.Width, last.Label.Position.Height);
                statusLabel.Label.Align = last.Label.Align;
                statusLabel.Label.Text = "状态";

                statusLabel.Text = new FreeText(font, color);
                statusLabel.Text.Position = new Rectangle(last.Text.Position.X, last.Text.Position.Y + dy, last.Text.Position.Width, last.Text.Position.Height);
                statusLabel.Text.Align = last.Text.Align;
                statusLabel.PropertyName = "DisplayStatus";
                
                this.troopDetail.LabelTexts.Add(statusLabel);
            }
