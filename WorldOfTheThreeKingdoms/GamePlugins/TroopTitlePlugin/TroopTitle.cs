        ...
        */
        ...
        internal void DrawTroop(Troop troop, bool playerControlling)
        {
            if (Session.Current.Scenario.ScenarioMap.TileWidth >= 50)
            {
                ...
                {
                    //UIKind = "New1";
                    // 使用新的头像路径获取方法，支持DDS、PNG、JPG格式
                    string path1 = GetTroopPortraitPath(this.PersonID, "Default");
                    //string path2 = GetTroopPortraitPath(this.PersonID, "Player");
        ...
        private Rectangle TheshiqiDisplayPosition(Troop troop)
        {
            ...
            {
                // 修复：确保士气条宽度至少为1像素
                int moraleWidth = Math.Max(1, troop.Army.Morale * this.Theshiqi2Position.Width / troop.Army.MoraleCeiling);
                return new Rectangle(this.DisplayOffset.X + this.Theshiqi2Position.X, this.DisplayOffset.Y + this.Theshiqi2Position.Y, moraleWidth, this.Theshiqi2Position.Height);
            }
            else
            {
                // 修复：确保士气条高度至少为1像素
                int moraleHeight = Math.Max(1, troop.Army.Morale * this.Theshiqi1Position.Height / troop.Army.MoraleCeiling);
                return new Rectangle(this.DisplayOffset.X + this.Theshiqi1Position.X, this.DisplayOffset.Y + this.Theshiqi1Position.Y+this.Theshiqi1Position.Height - moraleHeight, this.Theshiqi1Position.Width, moraleHeight);
            }       
        }
        private Rectangle ThezhanyiDisplayPosition(Troop troop)
        {
            ...
            {
                // 修复：确保战意条宽度至少为1像素
                int combativityWidth = Math.Max(1, troop.Army.Combativity * this.Thezhanyi2Position.Width / troop.Army.CombativityCeiling);
                return new Rectangle(this.DisplayOffset.X + this.Thezhanyi2Position.X, this.DisplayOffset.Y + this.Thezhanyi2Position.Y, combativityWidth, this.Thezhanyi2Position.Height);
            }
            else
            {
                // 修复：确保战意条高度至少为1像素
                int combativityHeight = Math.Max(1, troop.Army.Combativity * this.Thezhanyi1Position.Height / troop.Army.CombativityCeiling);
                return new Rectangle(this.DisplayOffset.X + this.Thezhanyi1Position.X, this.DisplayOffset.Y + this.Thezhanyi1Position.Y + this.Thezhanyi1Position.Height - combativityHeight, this.Thezhanyi1Position.Width, combativityHeight);
            }  
        }
        private Rectangle TheshiqiPosition(Troop troop)
        {
            if (UIKind == "New2")
            {
                // 修复：确保士气条宽度至少为1像素
                int moraleWidth = Math.Max(1, troop.Army.Morale * this.Theshiqi2Position.Width / troop.Army.MoraleCeiling);
                return new Rectangle(0, 0, moraleWidth, this.Theshiqi2Position.Height);
            }
            else
            {
                // 修复：确保士气条高度至少为1像素
                int moraleHeight = Math.Max(1, troop.Army.Morale * this.Theshiqi1Position.Height / troop.Army.MoraleCeiling);
                return new Rectangle(0, this.Theshiqi1Position.Height - moraleHeight, this.Theshiqi1Position.Width, moraleHeight);
            }                
        ...
        private Rectangle ThezhanyiPosition(Troop troop)
        {
            if (UIKind == "New2")
            {
                // 修复：确保战意条宽度至少为1像素
                int combativityWidth = Math.Max(1, troop.Army.Combativity * this.Thezhanyi2Position.Width / troop.Army.CombativityCeiling);
                return new Rectangle(0, 0, combativityWidth, this.Thezhanyi2Position.Height);
            }
            else
            {
                // 修复：确保战意条高度至少为1像素
                int combativityHeight = Math.Max(1, troop.Army.Combativity * this.Thezhanyi1Position.Height / troop.Army.CombativityCeiling);
                return new Rectangle(0, this.Thezhanyi1Position.Height - combativityHeight, this.Thezhanyi1Position.Width, combativityHeight);
            }  
        ...
        /// <param name="directory">目录类型（Default或Player）</param>
        /// <returns>头像路径</returns>
        private string GetTroopPortraitPath(int personId, string directory)
        {
            var baseDir = $@"Content\Textures\GameComponents\PersonPortrait\Images\{directory}\";
            var extensions = new[] { ".dds", ".png", ".jpg" };
            
            foreach (var ext in extensions)
            {
                var path = $"{baseDir}{personId}t{ext}";
                if (Platforms.Platform.Current.FileExists(path))
                    return path;
            }
