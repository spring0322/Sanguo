    ...
    internal class ArchitectureSurvey
    {
        ...
        private Rectangle DominationBarDisplayPosition
        {
            get
            {
                if (this.ArchitectureToSurvey.Domination == 0 || this.ArchitectureToSurvey.DominationCeiling == 0)
                {
                    ...
                    return new Rectangle(this.DominationBarClient.X + this.DisplayOffset.X, this.DominationBarClient.Y + this.DisplayOffset.Y, 1, this.DominationBarClient.Height);
                }
                // 修复：确保统治度条宽度至少为1像素
                int dominationWidth = Math.Max(1, this.DominationBarClient.Width * this.ArchitectureToSurvey.Domination / this.ArchitectureToSurvey.DominationCeiling);
                return new Rectangle(this.DominationBarClient.X + this.DisplayOffset.X, this.DominationBarClient.Y + this.DisplayOffset.Y, dominationWidth, this.DominationBarClient.Height);
            }
        }
        private Rectangle DominationDisplayPosition
        {
            get
            {
                ...
                {
                    // 修复：返回1像素宽度而不是0，避免绘制区域无效错误
                    return new Rectangle(0, 0, 1, this.DominationBarClient.Height);
                }
                // 修复：确保统治度条宽度至少为1像素
                int dominationWidth = Math.Max(1, this.DominationBarClient.Width * this.ArchitectureToSurvey.Domination / this.ArchitectureToSurvey.DominationCeiling);
                return new Rectangle(0, 0, dominationWidth, this.DominationBarClient.Height);
            }
        }
        private Rectangle EnduranceBarDisplayPosition
        {
            get
            {
                ...
                {
                    // 修复：返回1像素宽度而不是0，避免绘制区域无效错误
                    return new Rectangle(this.EnduranceBarClient.X + this.DisplayOffset.X, this.EnduranceBarClient.Y + this.DisplayOffset.Y, 1, this.EnduranceBarClient.Height);

                }
                // 修复：确保耐久度条宽度至少为1像素
                int enduranceWidth = Math.Max(1, this.EnduranceBarClient.Width * this.ArchitectureToSurvey.Endurance / this.ArchitectureToSurvey.EnduranceCeiling);
                return new Rectangle(this.EnduranceBarClient.X + this.DisplayOffset.X, this.EnduranceBarClient.Y + this.DisplayOffset.Y, enduranceWidth, this.EnduranceBarClient.Height);
            }
        }
        private Rectangle EnduranceDisplayPosition
        {
            get
            {
                ...
                {
                    // 修复：返回1像素宽度而不是0，避免绘制区域无效错误
                    return new Rectangle(0, 0, 1, this.EnduranceBarClient.Height);
                }
                // 修复：确保耐久度条宽度至少为1像素
                int enduranceWidth = Math.Max(1, this.EnduranceBarClient.Width * this.ArchitectureToSurvey.Endurance / this.ArchitectureToSurvey.EnduranceCeiling);
                return new Rectangle(0, 0, enduranceWidth, this.EnduranceBarClient.Height);
            }
        }
        private Rectangle AgricultureBarDisplayPosition
        {
            get
            {
                ...
                {
                    // 修复：返回1像素宽度而不是0，避免绘制区域无效错误
                    return new Rectangle(this.AgricultureBarClient.X + this.DisplayOffset.X, this.AgricultureBarClient.Y + this.DisplayOffset.Y, 1, this.AgricultureBarClient.Height);
                }
                // 修复：确保农业条宽度至少为1像素
                int agricultureWidth = Math.Max(1, this.AgricultureBarClient.Width * this.ArchitectureToSurvey.Agriculture / this.ArchitectureToSurvey.AgricultureCeiling);
                return new Rectangle(this.AgricultureBarClient.X + this.DisplayOffset.X, this.AgricultureBarClient.Y + this.DisplayOffset.Y, agricultureWidth, this.AgricultureBarClient.Height);
            }
        }
        private Rectangle AgricultureDisplayPosition
        {
            get
            {
                ...
                {
                    // 修复：返回1像素宽度而不是0，避免绘制区域无效错误
                    return new Rectangle(0, 0, 1, this.AgricultureBarClient.Height);
                }
                // 修复：确保农业条宽度至少为1像素
                int agricultureWidth = Math.Max(1, this.AgricultureBarClient.Width * this.ArchitectureToSurvey.Agriculture / this.ArchitectureToSurvey.AgricultureCeiling);
                return new Rectangle(0, 0, agricultureWidth, this.AgricultureBarClient.Height);
            }
        }
        private Rectangle CommerceBarDisplayPosition
        {
            get
            {
                ...
                {
                    // 修复：返回1像素宽度而不是0，避免绘制区域无效错误
                    return new Rectangle(this.CommerceBarClient.X + this.DisplayOffset.X, this.CommerceBarClient.Y + this.DisplayOffset.Y, 1, this.CommerceBarClient.Height);
                }
                // 修复：确保商业条宽度至少为1像素
                int commerceWidth = Math.Max(1, this.CommerceBarClient.Width * this.ArchitectureToSurvey.Commerce / this.ArchitectureToSurvey.CommerceCeiling);
                return new Rectangle(this.CommerceBarClient.X + this.DisplayOffset.X, this.CommerceBarClient.Y + this.DisplayOffset.Y, commerceWidth, this.CommerceBarClient.Height);
            }
        }
        private Rectangle CommerceDisplayPosition
        {
            get
            {
                ...
                {
                    // 修复：返回1像素宽度而不是0，避免绘制区域无效错误
                    return new Rectangle(0, 0, 1, this.CommerceBarClient.Height);
                }
                // 修复：确保商业条宽度至少为1像素
                int commerceWidth = Math.Max(1, this.CommerceBarClient.Width * this.ArchitectureToSurvey.Commerce / this.ArchitectureToSurvey.CommerceCeiling);
                return new Rectangle(0, 0, commerceWidth, this.CommerceBarClient.Height);
            }
        }
        private Rectangle TechnologyBarDisplayPosition
        {
            get
            {
                ...
                {
                    // 修复：返回1像素宽度而不是0，避免绘制区域无效错误
                    return new Rectangle(this.TechnologyBarClient.X + this.DisplayOffset.X, this.TechnologyBarClient.Y + this.DisplayOffset.Y, 1, this.TechnologyBarClient.Height);
                }
                // 修复：确保技术条宽度至少为1像素
                int technologyWidth = Math.Max(1, this.TechnologyBarClient.Width * this.ArchitectureToSurvey.Technology / this.ArchitectureToSurvey.TechnologyCeiling);
                return new Rectangle(this.TechnologyBarClient.X + this.DisplayOffset.X, this.TechnologyBarClient.Y + this.DisplayOffset.Y, technologyWidth, this.TechnologyBarClient.Height);
            }
        }
        private Rectangle TechnologyDisplayPosition
        {
            get
            {
                ...
                {
                    // 修复：返回1像素宽度而不是0，避免绘制区域无效错误
                    return new Rectangle(0, 0, 1, this.TechnologyBarClient.Height);
                }
                // 修复：确保技术条宽度至少为1像素
                int technologyWidth = Math.Max(1, this.TechnologyBarClient.Width * this.ArchitectureToSurvey.Technology / this.ArchitectureToSurvey.TechnologyCeiling);
                return new Rectangle(0, 0, technologyWidth, this.TechnologyBarClient.Height);
            }
        }
        private Rectangle MoraleBarDisplayPosition
        {
            get
            {
                ...
                {
                    // 修复：返回1像素宽度而不是0，避免绘制区域无效错误
                    return new Rectangle(this.MoraleBarClient.X + this.DisplayOffset.X, this.MoraleBarClient.Y + this.DisplayOffset.Y, 1, this.MoraleBarClient.Height);
                }
                // 修复：确保士气条宽度至少为1像素
                int moraleWidth = Math.Max(1, this.MoraleBarClient.Width * this.ArchitectureToSurvey.Morale / this.ArchitectureToSurvey.MoraleCeiling);
                return new Rectangle(this.MoraleBarClient.X + this.DisplayOffset.X, this.MoraleBarClient.Y + this.DisplayOffset.Y, moraleWidth, this.MoraleBarClient.Height);
            }
        }
        private Rectangle MoraleDisplayPosition
        {
            get
            {
                ...
                {
                    // 修复：返回1像素宽度而不是0，避免绘制区域无效错误
                    return new Rectangle(0, 0, 1, this.MoraleBarClient.Height);
                }
                // 修复：确保士气条宽度至少为1像素
                int moraleWidth = Math.Max(1, this.MoraleBarClient.Width * this.ArchitectureToSurvey.Morale / this.ArchitectureToSurvey.MoraleCeiling);
                return new Rectangle(0, 0, moraleWidth, this.MoraleBarClient.Height);
            }
        }
