    ...
    public class Column
    {
        ...
        public string GetDataString(int index)
        {
            try
            {
                ...
                {
                    Person p = this.tabList.gameObjectList[index] as Person;
                    return (p != null && p.HasSkill(this.ItemID)) ? "○" : "×";
                }

                if (this.Name.Equals("HasStunt"))
                {
                    Person p = this.tabList.gameObjectList[index] as Person;
                    return (p != null && p.HasStunt(this.ItemID)) ? "○" : "×";
                }
                if (this.Name.Equals("TitleName"))
                {
                    Person p = this.tabList.gameObjectList[index] as Person;
                    return (p != null) ? p.TitleName(this.ItemID) : "";
                }
                if (this.Name.Equals("HasInfluenceKind"))
                {
                    Person p = this.tabList.gameObjectList[index] as Person;
                    return (p != null && p.HasInfluenceKind(this.ItemID)) ? "○" : "×";
                }
                if (this.Name.Equals("InfluenceKindValueByTreasure"))
                {
                    Person p = this.tabList.gameObjectList[index] as Person;
                    return (p != null) ? p.InfluenceKindValueByTreasure(this.ItemID).ToString() : "";
                }
        ...
        public void ResetAllTextures()
        {
            ...
            try
            {
                if (this.Editable)
                {
                    ...
                    {
                        object val = StaticMethods.GetPropertyValue(this.tabList.gameObjectList[num], this.Name);
