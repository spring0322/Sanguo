...
namespace GameObjects.TroopDetail
{
    ...
    public class CastTargetKindList : GameObjectList
    {
        public CastTargetKindList GetSelectedList(TroopCastTargetKind kind)
        {
            ...
            // 日期：2026-03-21
            // 原因：AOT 环境下 foreach (CastTargetKind in List<GameObject>) 隐式转换失败
            // 解决：使用 for 循环 + as 类型转换 + Fail Fast
            for (int i = 0; i < base.GameObjects.Count; i++)
            {
                CastTargetKind kind2 = base.GameObjects[i] as CastTargetKind;
                if (kind2 == null)
                {
                    throw new InvalidOperationException($"CastTargetKindList 中存在非 CastTargetKind 类型的对象：{base.GameObjects[i]?.GetType().Name ?? "null"}");
                }
