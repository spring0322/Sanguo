...
namespace GameObjects
{
    ...
    public class Person : GameObject
    {
        ...
        // 亲爱武将列表 (存ID)
        [DataMember]
        public List<int> ClosePersons = new List<int>();
        
        // 厌恶武将列表 (存ID)
        [DataMember]
        public List<int> HatedPersons = new List<int>();
        ...
        public void Init()
        {
            ...
            // 初始化新的关系列表
            if (ClosePersons == null) ClosePersons = new List<int>();
            if (HatedPersons == null) HatedPersons = new List<int>();
        ...
        public void DoEnhanceDiplomatic()
        {
            ...
            */
            ...
            if ((this.BelongedFaction != null) && (this.TargetArchitecture.BelongedFaction != null))
            {
                ...
                // 宝物加成逻辑
                if (this.DiplomaticGiftTreasure != null)
                {
                    // 宝物价值加成：每100价值+1点友好度
                    g += this.DiplomaticGiftTreasure.Worth / 100;
                    
                    // 对方君主喜好加成 (如果喜好该类宝物，效果加倍)
                    // 简略处理：直接给与额外5点
                    g += 5;

                    // 转移宝物所有权
                    this.DiplomaticGiftTreasure.BelongedPerson = this.TargetArchitecture.BelongedFaction.Leader;
                    this.TargetArchitecture.BelongedFaction.Leader.Treasures.Add(this.DiplomaticGiftTreasure);
                    // 注意：宝物应在出发时已从原势力移除
                    
                    this.DiplomaticGiftTreasure = null; // 清空引用
...
#pragma warning restore CS0108 // 'Person.Name' hides inherited member 'GameObject.Name'. Use the new keyword if hiding was intended.
        ...
        public Treasure DiplomaticGiftTreasure { get; set; } = null;
        ...
        */
        ...
        /// <returns>1=亲爱, -1=厌恶, 0=普通</returns>
        public int CheckRelation(Person target)
        {
            if (target == null) return 0;
            
            // 确保关系列表已初始化
            if (this.ClosePersons == null) this.ClosePersons = new List<int>();
            if (this.HatedPersons == null) this.HatedPersons = new List<int>();
            
            // 优先检查显式关系列表
            if (this.ClosePersons.Contains(target.ID)) return 1;
            if (this.HatedPersons.Contains(target.ID)) return -1;
            
            // 如果没有显式关系，回退到原有的关系值系统
            if (Session.GlobalVariables.EnablePersonRelations)
            {
                int relationValue = GetRelation(target);
                if (relationValue >= 80) return 1;   // 高好感度视为亲爱
                if (relationValue <= -50) return -1; // 低好感度视为厌恶
            }
