...
#pragma warning restore CS0067 // The event 'Troop.OnTroopLost' is never used
        ...
        */
        ...
        /// <summary>
        /// 优化的伤害计算公式 - 简化复杂的Math.Pow运算以提高性能
        /// </summary>
        private int CalculateOptimizedDamage(float offence, float defence)
        {
            try
            {
                ...
                // 原公式: Math.Pow(offence/defence, 0.62) * 1.16 * 500 * TroopDamageRate
                // 简化为: (offence * 500 * TroopDamageRate) / (defence + 100)
        ...
        /// 更新LOD等级 - 基于距离摄像机的距离
        /// </summary>
        public void UpdateLODLevel(float distanceSquared)
        {
            try
            {
                // LOD距离阈值（平方距离）
                const float HIGH_LOD_THRESHOLD = 150f * 150f;
                const float MEDIUM_LOD_THRESHOLD = 300f * 300f;
                const float LOW_LOD_THRESHOLD = 600f * 600f;

                LODLevel newLevel;
                if (distanceSquared <= HIGH_LOD_THRESHOLD)
                {
                    newLevel = LODLevel.High;
                }
                else if (distanceSquared <= MEDIUM_LOD_THRESHOLD)
                {
                    newLevel = LODLevel.Medium;
                }
                else if (distanceSquared <= LOW_LOD_THRESHOLD)
                {
                    newLevel = LODLevel.Low;
                }
                else
                {
                    newLevel = LODLevel.VeryLow;
                }

                // 只在LOD等级真正改变时更新，避免不必要的操作
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[UpdateLODLevel] 错误: {ex.Message}");
                CurrentLODLevel = LODLevel.High; // 出错时使用高质量
            }
        }
        ...
        private TroopDamage SendAttackDamage(Troop troop, bool counter)
        {
            ...
            // 优化后的简化伤害计算公式
            // 原始复杂公式: Math.Pow(damage.SourceOffence / (float)defence, 0.62) * 1.16 * 500 * Session.Parameters.TroopDamageRate
            // 简化为线性公式以提高性能
        ...
        public Animation StatusTileAnimation
        {
            get
            {
                switch (this.Status)
                {
                    ...
                    case TroopStatus.行军:
                        return Session.Current.Scenario.GameCommonData.AllTileAnimations.GetAnimation(7); // 使用混乱动画

                    case TroopStatus.攻击:
                        return Session.Current.Scenario.GameCommonData.AllTileAnimations.GetAnimation(28); // 使用挑衅动画

                    case TroopStatus.驻扎:
                        return Session.Current.Scenario.GameCommonData.AllTileAnimations.GetAnimation(4); // 使用埋伏动画
