...
class Program
{
    ...
    {
        Console.WriteLine("=== GameScenario 反序列化问题诊断 ===");
        Console.WriteLine();
        ...
        try
        {
            ...
            Console.WriteLine($"📄 文件前200字符: {content.Substring(0, Math.Min(200, content.Length))}");
            Console.WriteLine();
            ...
            try
            {
                ...
                if (root.ValueKind == JsonValueKind.Object)
                {
                    ...
                    // 检查关键属性
                    var keyProperties = new[] { "Persons", "Factions", "Architectures", "Troops", "States", "Regions", "Sections" };
                    foreach (var prop in keyProperties)
                    {
                        ...
                        if (exists && propElement.ValueKind == JsonValueKind.Object)
                        {
                            if (propElement.TryGetProperty("GameObjects", out var gameObjectsElement))
                            {
                                if (gameObjectsElement.ValueKind == JsonValueKind.Array)
                                {
                                    Console.WriteLine($"     - GameObjects 数组长度: {gameObjectsElement.GetArrayLength()}");
                                }
                                else if (gameObjectsElement.ValueKind == JsonValueKind.Object)
                                {
                                    Console.WriteLine($"     - GameObjects 对象属性数: {gameObjectsElement.EnumerateObject().Count()}");
                                }
                            }
                        }
                    }
            ...
            try
            {
                ...
                var result = JsonSerializer.Deserialize<GameScenario>(content, options);
                ...
                else
                {
                    ...
                    Console.WriteLine($"   - Persons: {result.Persons?.Count ?? 0}");
                    Console.WriteLine($"   - Factions: {result.Factions?.Count ?? 0}");
                    Console.WriteLine($"   - Architectures: {result.Architectures?.Count ?? 0}");
                    Console.WriteLine($"   - Troops: {result.Troops?.Count ?? 0}");
                    Console.WriteLine($"   - States: {result.States?.Count ?? 0}");
                    Console.WriteLine($"   - Regions: {result.Regions?.Count ?? 0}");
                    Console.WriteLine($"   - Sections: {result.Sections?.Count ?? 0}");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                var result = SimpleSerializer.DeserializeJson<GameScenario>(content);
