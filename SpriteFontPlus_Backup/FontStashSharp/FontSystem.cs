	...
	internal unsafe class FontSystem
	{
		...
		private FontGlyph GetGlyph(Font font, int codepoint, int isize, int iblur, int bitmapOption)
		{
			...
			if (g == 0)
			{
				...
				// 🔥 技术性修复：如果仍然没有找到字形，尝试替代字符
				if (g == 0)
				{
					System.Diagnostics.Debug.WriteLine($"[FontSystem] 字形未找到: U+{codepoint:X4} ('{(char)codepoint}')");

					// 对于数字字符，尝试使用问号作为替代
					if (codepoint >= '0' && codepoint <= '9')
					{
						g = font._font.fons__tt_getGlyphIndex((int)'?');
						if (g == 0)
						{
							// 如果问号也没有，使用空格
							g = font._font.fons__tt_getGlyphIndex((int)' ');
					...
					// 如果连空格都没有，返回null（不渲染这个字符）
					if (g == 0)
					{
						System.Diagnostics.Debug.WriteLine($"[FontSystem] 无法找到任何替代字形，跳过字符: U+{codepoint:X4}");
						return null;
					}
				}
			}
		...
		private void Flush(SpriteBatch batch, float depth)
		{
            ...
            {
                 System.Diagnostics.Debug.WriteLine("[FontSystem.Flush] Batch or GraphicsDevice is null!");
                 return;
            }
            
            try
            {
                // [Diagnostic] Check for Device Mismatch or Disposal
                if (batch.GraphicsDevice.IsDisposed)
                {
                     System.Diagnostics.Debug.WriteLine($"[FontSystem.Flush] CRITICAL: Batch GraphicsDevice is DISPOSED! ID: {batch.GraphicsDevice.GetHashCode()}");
                     return; 
                }

                if (Texture == null || Texture.IsDisposed) 
                {
                    System.Diagnostics.Debug.WriteLine($"[FontSystem.Flush] Creating new Texture. Batch Device ID: {batch.GraphicsDevice.GetHashCode()}"); 
                    Texture = new Texture2D(batch.GraphicsDevice, _params_.Width, _params_.Height);
                    System.Diagnostics.Debug.WriteLine($"[FontSystem.Flush] Texture created successfully. Width: {_params_.Width}, Height: {_params_.Height}");
                }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[FontSystem.Flush] Exception: {ex.Message}");
                _vertsNumber = 0;
            }
		}
